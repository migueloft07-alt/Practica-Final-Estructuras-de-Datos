...
int main() {
    ...
    // ════════════════════════════════════════════════════════════════════════
    titulo("SECCION 7 — EXPORTACION DE DATASETS");

    // Determinar la carpeta donde esta el ejecutable para crear datasets/ ahi
    std::string carpeta_base = "";
#if defined(_WIN32) || defined(_WIN64)
    char ruta_exe[MAX_PATH];
    if (GetModuleFileNameA(NULL, ruta_exe, MAX_PATH) > 0) {
        std::string ruta(ruta_exe);
        size_t pos = ruta.find_last_of("\\/");
        if (pos != std::string::npos)
            carpeta_base = ruta.substr(0, pos + 1);
    }
#endif
    const std::string dir_datasets = carpeta_base + "datasets";

    // Crear la carpeta datasets si no existe
#if defined(_WIN32) || defined(_WIN64)
    CreateDirectoryA(dir_datasets.c_str(), NULL);
#else
    system(("mkdir -p " + dir_datasets).c_str());
#endif

    std::cout << "  Carpeta de salida: " << dir_datasets << "\n\n";
    ...
    for (size_t n : std::vector<size_t>{100'000, 1'000'000})
        for (int U : Us)
            for (const auto& dist : dists) {
                ...
                const std::string fname = dir_datasets + "/dataset_n"
                    + std::to_string(n) + "_U" + std::to_string(U)
                    + "_" + dist.nombre + ".csv";
                std::ofstream f(fname);
                if (!f.is_open()) {
                    std::cerr << "  [ERROR] No se pudo crear: " << fname << "\n";
                    continue;
                }
                f << "indice,valor\n";
